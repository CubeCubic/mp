  ...
  contactForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        // Успех
        if (contactStatus) {
          contactStatus.textContent = 'წერილი წარმატებით გაიგზავნა! მადლობა!';
          contactStatus.className = 'contact-status success';
        }
        
        // Очистить форму через 2 секунды и закрыть
        setTimeout(() => {
          closeContactModal();
        }, 2000);
      } else {
        // Ошибка
        throw new Error(result.message || 'შეცდომა გაგზავნისას');
      }
    } catch (error) {
      ...
      // Показать ошибку
      if (contactStatus) {
        contactStatus.textContent = 'შეცდომა: ვერ გაიგზავნა. სცადეთ თავიდან.';
