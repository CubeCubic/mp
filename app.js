...
// --- Rating Functions (Add at top of app.js, after variable declarations) ---
...
toRender.forEach(t => {
  ...
  // --- Add rating UI to card ---
  const ratingDiv = document.createElement('div');
  ratingDiv.className = 'rating-stars';
  ratingDiv.dataset.trackId = t.id;
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    star.addEventListener('click', (ev) => {
      ...
      if (!isNaN(val)) {
        saveRating(t.id, val);
        updateStarDisplay(ratingDiv, val);
      }
    });
  ...
  // Load and show saved rating
  const saved = loadRating(t.id);
  updateStarDisplay(ratingDiv, saved);
